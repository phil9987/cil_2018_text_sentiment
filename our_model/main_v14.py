...
model v12: v8 with wider hidden state; directly 4 epochs; smaller eval percentage (0.5% approx 10K tweets)
model v13: v8 without pool layers; smaller eval percentage, too.
model v14: v8 with removed word order in the tweets; vectors are sorted by vector length
...
v14.0 2018-06-29 Group The Optimists
"""
...
def vector_len(vector):
    '''note: simple implementation without focus on math robustness (would need sorting and other parts)'''
    vecsum = 0.0
    for v in vector:
        vecsum += v * v
...
def load_embeddings_helper():
    '''embeddings: tuple of (vector_len, initial_index, word, embedding_vector)
       note: initial_index is provided for debugging purposes to find original location after sorting'''
    embeddings = []
    index = 0
    with open('../data/glove.twitter.27B/glove.twitter.27B.{}d.txt'.format(DIM), 'r') as file:
        for line in file:
            ...
            word_weights = to_float(values[1:])
            if len(word_weights) != DIM:
                print('wrong encoding length {} for ""; word ignored'.format(len(word_weights), word))
                continue
            embeddings.append((vector_len(word_weights), index, word, word_weights))
            index += 1

    embeddings.sort(key=get_sort_key, reverse=True)
    return embeddings
...
def load_embeddings():
    '''
        ...
        v14: embedding indices are sorted by vector length,
             thus, sorting the tweets by index will be good enough without looking into the
             vectors all the time
    '''
    sorted_embeddings = load_embeddings_helper()
...
def tweet_encoding(context, tweet, word2idx, is_training):
    '''
        ...
        v14: use sorted word list to remove temporal dependency of words
    '''
    word_list = []
    pad = word2idx[PAD]
    ...
    # depends on the prepared order of indices (see loading of embedding)
    word_list.sort()
...
def main():
    ...
    if TRAIN or EVALUATE or PREDICT:
        ...
        n = len(X_lengths)
        eval_n = int(n / 200)        # use 0.5% of the data for evaluation
